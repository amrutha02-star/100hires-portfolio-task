...
- Used one small open source tool consistently. I pulled transcripts with youtube-transcript-api, run locally on my Mac rather than as a script saved in this repo, and saved every file the same way, with the source link and the date I pulled it, so anyone can trace it back and I can repeat the same process for the next expert.
